...
const Stats = () => {
  ...
  return (
    <section className="flex flex-col sm:flex-row sm:flex-wrap gap-5 mt-4">
      {stats.map((stat, index) => (
        <div
          key={index}
          className={`w-full sm:flex-1 min-w-[200px] shadow-lg rounded-xl p-6 flex items-center gap-4 transition-transform transform hover:scale-105 ${stat.color}`}
        >
